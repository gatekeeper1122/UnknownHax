...
if not menu.is_trusted_mode_enabled() then
    menu.notify("Heist Control requires Trusted Mode to be activated", "Enable the Trusted Mode", 8, 2552550)
    return
end
--
-- Script function
...
local HC2_MAIN = menu.add_feature("» Heist Control v2.7.0", "parent", 0, function()
end)
...
local CP_DEFAULT = menu.add_feature("» Random Preset", "parent", MNCP_.id)

local CPPR = menu.add_feature("» Normal Presets (Payout: $2.5 millions)", "parent", MNCP_.id)

local MNCP_EV_ = menu.add_feature("» Weekly Event Presets (Payout: $4.1 millions)", "parent", MNCP_.id)
...
local CONTRACT_LS = menu.add_feature("» LS Tuners Contracts", "parent", HC2_MAIN.id)
...
local DEFAULT_METHOD = {
    ...
    {"H4LOOT_CASH_I", 0},
    {"H4LOOT_CASH_C", 0},
    {"H4LOOT_WEED_I", 5588700},
    {"H4LOOT_WEED_C", 90},
    {"H4LOOT_COKE_I", 11188515},
    {"H4LOOT_COKE_C", 0},
    {"H4LOOT_GOLD_I", 0},
    {"H4LOOT_GOLD_C", 165},
    {"H4LOOT_PAINT", 127},
    ...
    {"H4LOOT_WEED_I_SCOPED", 5588700},
    {"H4LOOT_WEED_C_SCOPED", 90},
    {"H4LOOT_COKE_I_SCOPED", 11188515},
    {"H4LOOT_COKE_C_SCOPED", 0},
    {"H4LOOT_GOLD_I_SCOPED", 0},
    {"H4LOOT_GOLD_C_SCOPED", 165},
    {"H4LOOT_PAINT_SCOPED", 127},
    {"H4_MISSIONS", 65535},
    {"H4_PLAYTHROUGH_STATUS", 1}

}

local DEFAULT_METHOD_TARGET_SET = {

    {"H4CNF_TARGET", 1,5,1,5},
    {"H4CNF_WEAPONS", 1,5,1,5},
    {"H4CNF_TROJAN", 1,5,1,5}
}

    menu.add_feature("» Load Random Preset", "action", CP_DEFAULT.id, function()
        menu.notify("Note: Payments, targets and weapons are random.\n\nCayo Perico Heist is ready to play", "Heist Control", 5, 780000)
        for i = 1, #DEFAULT_METHOD_TARGET_SET do
        stat_set_int(DEFAULT_METHOD_TARGET_SET[i][1], true, math.random(DEFAULT_METHOD_TARGET_SET[i][4], DEFAULT_METHOD_TARGET_SET[i][5]))
        for i = 2, #DEFAULT_METHOD do 
        stat_set_int(DEFAULT_METHOD[i][1], true, DEFAULT_METHOD[i][2])
        end
    end
end)
end
...
menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", SOLO0.id, function()
    menu.notify("Preset calculated for SOLO (Normal)\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
menu.add_feature("» Sinsimito Tequila : $990,000 (Hard)", "action", SOLO0.id, function()
    menu.notify("Preset calculated for SOLO (Hard)\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
menu.add_feature("» Panther Statue : $1,900,000 (Normal)", "action", SOLO0.id, function()
    menu.notify("Preset calculated for SOLO (Normal)\n\nPrimary Target: Sapphire Panther\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
local MNCP_D20 = menu.add_feature("» 2 Players [Set 50% to each]", "parent", CPPR.id)

menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", MNCP_D20.id, function()
    menu.notify("Preset calculated for 2 Players (Normal)\n\nMandatory: Set 50% for each\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
menu.add_feature("» Sinsimito Tequila : $990,000 (Hard)", "action", MNCP_D20.id, function()
    menu.notify("Preset calculated for 2 Players (Hard)\n\nMandatory: Set 50% for each\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
local NEW_METHOD_2PLYRs = menu.add_feature("» 2 Players [Host: 15% & Player2: 85%]", "parent", CPPR.id)

menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", NEW_METHOD_2PLYRs.id, function()
    menu.notify("Preset calculated for 2 Players (Normal)\n\nMandatory: Host 15% and 85% Player2\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
    menu.notify("Note: The host will receive much less", "Heist Control", 8, 780000)
...
-- 3P Method MENU
local MNCP_D30 = menu.add_feature("» 3 Players [Host: 20% & 2 Players: 40%]", "parent", CPPR.id)

menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", MNCP_D30.id, function()
    menu.notify("Preset calculated for 3 Players (Normal)\n\nMandatory: Host 20% and 40% to others two players\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
menu.add_feature("» Sinsimito Tequila : $990,000 (Hard)", "action", MNCP_D30.id, function()
    menu.notify("Preset calculated for 3 Players (Hard)\n\nMandatory: Host 20% and 40% to others two players\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
local FCP_SET_4P_METHOD_0_TQL = {
    ...
    {"H4CNF_GRAPPEL", 5156},
    {"H4CNF_APPROACH", -1},
    {"H4LOOT_CASH_I", 9718308},
    {"H4LOOT_CASH_C", 0},
    {"H4LOOT_WEED_I", 6336841},
    {"H4LOOT_WEED_C", 0},
    {"H4LOOT_COKE_I", 722066},
    {"H4LOOT_COKE_C", 0},
    {"H4LOOT_GOLD_I", 0},
    {"H4LOOT_GOLD_C", 255},
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 126823},
    {"H4LOOT_CASH_V", 659090},
    {"H4LOOT_COKE_V", 1318181},
    {"H4LOOT_GOLD_V", 1757575},
    {"H4LOOT_PAINT_V", 1318181},
    {"H4LOOT_WEED_V", 1054545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
    {"H4LOOT_COKE_I_SCOPED", 722066},
    {"H4LOOT_COKE_C_SCOPED", 0},
    {"H4LOOT_GOLD_I_SCOPED", 0},
    {"H4LOOT_GOLD_C_SCOPED", 255},
    {"H4LOOT_PAINT_SCOPED", 127},
...
local FCP_SET_4P_METHOD_0_RBY = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 126823},
    {"H4LOOT_CASH_V", 652840},
    {"H4LOOT_COKE_V", 1305681},
    {"H4LOOT_GOLD_V", 1740908},
    {"H4LOOT_PAINT_V", 1305681},
    {"H4LOOT_WEED_V", 1044545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_0_BRB = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 126823},
    {"H4LOOT_CASH_V", 646590},
    {"H4LOOT_COKE_V", 1293181},
    {"H4LOOT_GOLD_V", 1724242},
    {"H4LOOT_PAINT_V", 1293181},
    {"H4LOOT_WEED_V", 1034545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_0_PKD = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 126823},
    {"H4LOOT_CASH_V", 634090},
    {"H4LOOT_COKE_V", 1268181},
    {"H4LOOT_GOLD_V", 1690908},
    {"H4LOOT_PAINT_V", 1268181},
    {"H4LOOT_WEED_V", 1014545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_0_PTS = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 126823},
    {"H4LOOT_CASH_V", 596590},
    {"H4LOOT_COKE_V", 1193181},
    {"H4LOOT_GOLD_V", 1590908},
    {"H4LOOT_PAINT_V", 1193181},
    {"H4LOOT_WEED_V", 954545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_H0_TQL = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 131055},
    {"H4LOOT_CASH_V", 653465},
    {"H4LOOT_COKE_V", 1306931},
    {"H4LOOT_GOLD_V", 1742575},
    {"H4LOOT_PAINT_V", 1306931},
    {"H4LOOT_WEED_V", 1045545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_H0_BRB = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 131055},
    {"H4LOOT_CASH_V", 639715},
    {"H4LOOT_COKE_V", 1279431},
    {"H4LOOT_GOLD_V", 1705908},
    {"H4LOOT_PAINT_V", 1279431},
    {"H4LOOT_WEED_V", 1023545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_H0_PKD = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 131055},
    {"H4LOOT_CASH_V", 625965},
    {"H4LOOT_COKE_V", 1251931},
    {"H4LOOT_GOLD_V", 1669242},
    {"H4LOOT_PAINT_V", 1251931},
    {"H4LOOT_WEED_V", 1001545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local FCP_SET_4P_METHOD_H0_PTS = {
    ...
    {"H4LOOT_PAINT", 127},
    {"H4_PROGRESS", 131055},
    {"H4LOOT_CASH_V", 584715},
    {"H4LOOT_COKE_V", 1169431},
    {"H4LOOT_GOLD_V", 1559242},
    {"H4LOOT_PAINT_V", 1169431},
    {"H4LOOT_WEED_V", 935545},
    {"H4LOOT_CASH_I_SCOPED", 9718308},
    {"H4LOOT_CASH_C_SCOPED", 0},
    {"H4LOOT_WEED_I_SCOPED", 6336841},
    {"H4LOOT_WEED_C_SCOPED", 0},
...
local MNCP_D40 = menu.add_feature("» 4 Players [Set 25% to each]", "parent", CPPR.id)

menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", MNCP_D40.id, function()
    menu.notify("Preset calculated for 4 Players (Normal)\n\nMandatory: Set 25% to everyone\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
menu.add_feature("» Sinsimito Tequila : $990,000 (Hard)", "action", MNCP_D40.id, function()
    menu.notify("Preset calculated for 4 Players (Hard)\n\nMandatory: Set 25% to everyone\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $2,550,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control", 9, 257818)
...
do
    ...
    menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", MNCP_EV_SL_.id, function()
        menu.notify("Preset calculated for SOLO (Normal)\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $4,100,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control - Weekly Event Preset", 9, 257818)
        menu.notify("This method should only be used when Rockstar activates the bonus event, always check Tuesday/Thursday on the newswire", "Rockstar Newswire", 9, 780000)
...
local MNCP_D20_EV = menu.add_feature("» 2 Players [Set 50% to each]", "parent", MNCP_EV_.id)

menu.add_feature("» Sinsimito Tequila : $900,000 (Normal)", "action", MNCP_D20_EV.id, function()
    menu.notify("Preset calculated for 2 Players (Normal)\n\nMandatory: 50% Host & 50% Player2\n\nPrimary Target: Sinsimito Tequila\nSecondary Target: Mixed\nEstimated payout: $4,100,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control - Weekly Event Preset", 9, 257818)
    menu.notify("This method should only be used when Rockstar activates the bonus event, always check Tuesday/Thursday on the newswire", "Rockstar Newswire", 9, 780000)
...
local MNCP_D30_EV = menu.add_feature("» 3 Players [Host: 20% & 2 Players: 40%]", "parent", MNCP_EV_.id)
...
menu.add_feature("» Minimadrazzo Files : $1,100,000 (Normal)", "action", MNCP_D30_EV.id, function()
    menu.notify("Preset calculated for 3 Players (Normal)\n\nMandatory: Host 20% and 40% to others two players\n\nPrimary Target: Minimadrazzo Files\nSecondary Target: Mixed\nEstimated payout: $4,100,000\n\nWeapon Loadout: Marksman\nVehicle Choosed: All avaliable", "Heist Control - Weekly Event Preset", 9, 257818)
    menu.notify("This method should only be used when Rockstar activates the bonus event, always check Tuesday/Thursday on the newswire", "Rockstar Newswire", 9, 780000)
...
local MNCP_D40_EV = menu.add_feature("» 4 Players [Set 25% to each]", "parent", MNCP_EV_.id)
...
menu.add_feature("» Submarine: KOSATKA", "action", sub_option_A2.id, function()
    menu.notify("KOSATKA avaliable", "Heist Control", 5, 257818)
...
menu.add_feature("» Plane: ALKONOST", "action", sub_option_A2.id, function()
    menu.notify("ALKONOST avaliable", "Heist Control", 5, 257818)
...
menu.add_feature("» Plane: VELUM", "action", sub_option_A2.id, function()
    menu.notify("VELUM avaliable", "Heist Control", 5, 257818)
...
menu.add_feature("» Helicopter: STEALTH ANNIHILATOR", "action", sub_option_A2.id, function()
    menu.notify("STEALTH ANNIHILATOR avaliable", "Heist Control", 5, 257818)
...
menu.add_feature("» Boat: PATROL BOAT", "action", sub_option_A2.id, function()
    menu.notify("PATROL BOAT avaliable", "Heist Control", 5, 257818)
...
menu.add_feature("» Boat: LONGFIN", "action", sub_option_A2.id, function()
    menu.notify("LONGFIN avaliable", "Heist Control", 5, 257818)
...
menu.add_feature("» Unlock All Vehicles", "action", sub_option_A2.id, function()
    menu.notify("All Vehicles are avaliable", "Heist Control", 5, 257818)
...
-- PART 2
    ...
    menu.notify("Secondary Target are now Mixed\n\nWhen using this method, the percentage and final payment is random!", "Heist Control", 5, 257818)
    ...
    menu.notify("Secondary Target has been removed", "Heist Control", 5, 0x00FF00)
    for i = 1, #SecondaryT_Remove do
    stat_set_int(SecondaryT_Remove[i][1], true, SecondaryT_Remove[i][2])
end
end)

local compound_options = menu.add_feature("» Compound Loot", "parent", sub_option_C0.id)

    menu.add_feature("» Change to Mixed Loot", "action", compound_options.id, function()
    menu.notify("Compound Loot has been modified", "Heist Control", 5, 0x00FF00)
    for i = 1, #Compound_LT_MIX do
    stat_set_int(Compound_LT_MIX[i][1], true, Compound_LT_MIX[i][2])
    end
end)

    menu.add_feature("» Change to full Cash", "action", compound_options.id, function()
    menu.notify("Compound Loot modified to Cash", "Heist Control", 5, 0x00FF00)
    for i = 1, #Compound_LT_CASH do
    stat_set_int(Compound_LT_CASH[i][1], true, Compound_LT_CASH[i][2])
    end
end)

    menu.add_feature("» Change to full Weed", "action", compound_options.id, function()
    menu.notify("Compound Loot modified to Weed", "Heist Control", 5, 0x00FF00)
    for i = 1, #Compound_LT_WEED do
    stat_set_int(Compound_LT_WEED[i][1], true, Compound_LT_WEED[i][2])
    end
end)

    menu.add_feature("» Change to full Coke", "action", compound_options.id, function()
        menu.notify("Compound Loot modified to Coke", "Heist Control", 5, 0x00FF00)
        for i = 1, #Compound_LT_COKE do
        stat_set_int(Compound_LT_COKE[i][1], true, Compound_LT_COKE[i][2])
        end
    end)

    menu.add_feature("» Change to full Gold", "action", compound_options.id, function()
    menu.notify("Compound Loot modified to Gold", "Heist Control", 5, 	0x00FF00)
    for i = 1, #Compound_LT_GOLD do
    stat_set_int(Compound_LT_GOLD[i][1], true, Compound_LT_GOLD[i][2])
end
end)

    menu.add_feature("» Change to full Paint", "action", compound_options.id, function()
    menu.notify("Compound Loot modified to Paint", "Heist Control", 5, 	0x00FF00)
    for i = 1, #Compound_LT_PAINT do
    stat_set_int(Compound_LT_PAINT[i][1], true, Compound_LT_PAINT[i][2])
end
end)

    menu.add_feature("» Remove Paint (only)", "action", compound_options.id, function()
    menu.notify("Paints has been removed", "Heist Control", 5, 	0x00FF00)
    for i = 1, #Remove_Compound_Paint do
    stat_set_int(Remove_Compound_Paint[i][1], true, Remove_Compound_Paint[i][2])
end
end)

    menu.add_feature("» Remove all Compound Loot", "action", compound_options.id, function()
    menu.notify("Compound loots has been removed", "Heist Control", 5, 	0x00FF00)
...
menu.add_feature("» Conspirator Loadout", "action", sub_option_A2.id, function()
    menu.notify("Conspirator Loadout\n\nMilitary Rifle + AP\nKnuckles + Stickies", "Heist Control", 5, 257818)
...
menu.add_feature("» Crackshot Loadout", "action", sub_option_A2.id, function()
    menu.notify("Crackshot Loadout\n\nSniper + AP\nKnife + Molotov", "Heist Control", 5, 257818)
...
menu.add_feature("» Saboteur Loadout", "action", sub_option_A2.id, function()
    menu.notify("Saboteur Loadout\n\nSMG Mk2 + SNS Pistol\nKnife + Pipe Bomb", "Heist Control", 5, 257818)
...
menu.add_feature("» Marksman Loadout", "action", sub_option_A2.id, function()
    menu.notify("Marksman Loadout\n\nAK-47 + Pistol .50\nMachete + Pipe Bomb", "Heist Control", 5, 257818)
...
local ITM_SPWN = menu.add_feature("» Equipments Spawn Location", "parent", CPMN.id)

menu.add_feature("» Set to Equipments spawn next to Airport", "action", ITM_SPWN.id, function()
    menu.notify("Such items will spawn next to Airport:\n\nGrappling Hook\nGuard Clothing\nBolt Cutters", "Heist Control", 5, 257818)
    for i = 1, #CP_Item_SpawnPlace_AIR do
        stat_set_int(CP_Item_SpawnPlace_AIR[i][1], true, CP_Item_SpawnPlace_AIR[i][2])
    end
end)

menu.add_feature("» Set to Equipments spawn next to Docks", "action", ITM_SPWN.id, function()
    menu.notify("Such items will spawn next to Docks:\n\nGrappling Hook\nGuard Clothing\nBolt Cutters", "Heist Control", 5, 257818)
    for i = 1, #CP_Item_SpawnPlace_DKS do
        stat_set_int(CP_Item_SpawnPlace_DKS[i][1], true, CP_Item_SpawnPlace_DKS[i][2])
    end
end)

menu.add_feature("» Set to Equipments spawn next to Compound", "action", ITM_SPWN.id, function()
        menu.notify("Such items will spawn next to Compound:\n\nGrappling Hook\nGuard Clothing\nBolt Cutters", "Heist Control", 5, 257818)
...
do
    ...
    menu.add_feature("» Set to Supply Truck spawn at Airport", "action", TRCK_SPWN.id, function()
    menu.notify("Supply Truck will now spawn next to Airport", "Heist Control", 5, 257818)
    for i = 1, #CP_TRUCK_SPAWN_mov1 do
    stat_set_int(CP_TRUCK_SPAWN_mov1[i][1], true, CP_TRUCK_SPAWN_mov1[i][2])
end
end)
    
    menu.add_feature("» Set to Supply Truck spawn at North Dock", "action", TRCK_SPWN.id, function()
    menu.notify("Supply Truck will now spawn next to North Dock", "Heist Control", 5, 257818)
    for i = 1, #CP_TRUCK_SPAWN_mov2 do
    stat_set_int(CP_TRUCK_SPAWN_mov2[i][1], true, CP_TRUCK_SPAWN_mov2[i][2])
end
end)
    
    menu.add_feature("» Set to Supply Truck spawn at Main Dock - East", "action", TRCK_SPWN.id, function()
    menu.notify("Supply Truck will now spawn next to Main Dock - East", "Heist Control", 5, 257818)
    for i = 1, #CP_TRUCK_SPAWN_mov3 do
    stat_set_int(CP_TRUCK_SPAWN_mov3[i][1], true, CP_TRUCK_SPAWN_mov3[i][2])
end
end)
    
    menu.add_feature("» Set to Supply Truck spawn at Main Dock - West", "action", TRCK_SPWN.id, function()
    menu.notify("Supply Truck will now spawn next to Main Dock - West", "Heist Control", 5, 257818)
    for i = 1, #CP_TRUCK_SPAWN_mov4 do
    stat_set_int(CP_TRUCK_SPAWN_mov4[i][1], true, CP_TRUCK_SPAWN_mov4[i][2])
end
end)
    
    menu.add_feature("» Set to Supply Truck spawn next to Compound", "action", TRCK_SPWN.id, function()
    menu.notify("Supply Truck will now spawn next to Compound", "Heist Control", 5, 257818)
...
do
    ...
    menu.add_feature("» Change Difficulty to Normal", "action", sub_option_A0.id, function()
        menu.notify("Heist Difficulty has been changed to Normal", "Difficulty Editor", 6, 257818)
    ...
    menu.add_feature("» Change Difficulty to Hard", "action", sub_option_A0.id, function()
        menu.notify("Heist Difficulty has been changed to Hard", "Difficulty Editor", 6, 257818)
...
local MR0_OPTIONS = menu.add_feature("» More Options", "parent", CPMN.id)

-- Cayo Perico Awards
    menu.add_feature("» Unlock Cayo Perico Awards", "action", MR0_OPTIONS.id, function()
    menu.notify("Cayo Perico Awards Unlocked!", "Heist Control", 5, 257818)
...
---
    ...
    menu.add_feature("» Complete all Missions only", "action", MR0_OPTIONS.id, function()
        menu.notify("All missions are completed!", "Heist Control", 5, 257818)
...
---
    ...
    menu.add_feature("» Force the longest final Cutscene", "action", MR0_OPTIONS.id, function()
        menu.notify("Keep in mind that you must use this option before starting the Heist\n\nDone!", "Heist Control", 5, 257818)
...
----
    ...
    menu.add_feature("» Set Heist to Default (Reset)", "action", MR0_OPTIONS.id, function()
        menu.notify("Heist has been restored", "Heist Control", 6, 257818)
    for i = 1, #CP_RST do
...
---
    ...
    menu.add_feature("» Remove Repeat Cooldown", "action", MR0_OPTIONS.id, function()
        menu.notify("Alert: This is NOT a bypass for the Server-Side Cooldown (Payout)\n\nPlease wait up to 15 minutes to avoid not receiving the money in the end", "Heist Control", 7, 780000)
    for i = 1, #CLD_RMV do
...
do
    ...
    menu.add_feature("» Load Random Approach", "action", CH_0sub.id, function()
        menu.notify("Make sure you have paid the heist on the planning screen before using this option\n\nRandom Preset Loaded!", "Heist Control", 5, 780000)
        for i = 1, #CH_RANDOM_PRST do
            stat_set_int(CH_RANDOM_PRST[i][1], true, CH_RANDOM_PRST[i][2])
        for i = 2, #CH_ADDON_1 do          
            stat_set_int(CH_ADDON_1[i][1], true, math.random(CH_ADDON_1[i][4], CH_ADDON_1[i][5]))
        end
    end
end)
    ...
    menu.add_feature("» Silent & Sneaky Approach (Normal)", "action", CH_0sub.id, function()
        menu.notify("Silent & Sneaky Approach (Normal)\n\nTarget: Diamond\nVehicle: Everon\nDriver Crew: Chester McCoy\n\nWeapon: Rifle + Shotgun\nGunman: Gustavo Mota\n\nHacker: Avi Schwartzman\nUndetected: 3 minutes 30s\nDetected: 2 minutes 26s\n\nMask: Hunter Set", "Heist Control", 7, 257818)
    ...
    menu.add_feature("» BigCon Approach (Hard)", "action", CH_0sub.id, function()
        menu.notify("BigCon Approach (Hard)\n\nTarget: Diamond\nVehicle: Everon\nDriver Crew: Chester McCoy\n\nWeapon: Rifle + Pistol\nGunman: Gustavo Mota\n\nHacker: Avi Schwartzman\nUndetected: 3 minutes 30s\nDetected: 2 minutes 26s\n\nMask: Hunter Set", "Heist Control", 7, 257818)
    ...
    menu.add_feature("» BigCon Approach (Normal)", "action", CH_0sub.id, function()
        menu.notify("BigCon Approach (Normal)\n\nTarget: Diamond\nVehicle: Everon\nDriver Crew: Chester McCoy\n\nWeapon: Rifle + Pistol\nGunman: Gustavo Mota\n\nHacker: Avi Schwartzman\nUndetected: 3 minutes 30s\nDetected: 2 minutes 26s\n\nMask: Hunter Set", "Heist Control", 7, 257818)
    ...
    menu.add_feature("» Aggressive Approach (Hard)", "action", CH_0sub.id, function()
        menu.notify("Aggressive Approach (Hard)\n\nTarget: Diamond\nVehicle: Everon (4 Players)\nDriver Crew: Chester McCoy\n\nWeapon: Submachine Gun + Shotgun\nGunman: Gustavo Mota\n\nHacker: Avi Schwartzman\nUndetected: 3 minutes 30s\nDetected: 2 minutes 26s\n\nMask: Hunter Set", "Heist Control", 7, 257818)
        ...
        menu.notify("Aggressive Approach (Normal)\n\nTarget: Diamond\nVehicle: Everon (4 Players)\nDriver Crew: Chester McCoy\n\nWeapon: Submachine Gun + Shotgun\nGunman: Gustavo Mota\n\nHacker: Avi Schwartzman\nUndetected: 3 minutes 30s\nDetected: 2 minutes 26s\n\nMask: Hunter Set", "Heist Control", 7, 257818)
...
do
    ...
    local CH_1sub = menu.add_feature("» Heist Planning (Board 1)", "parent", CHMN.id)

    --- CH - Unlocker Related
    menu.add_feature("» Unlock all Points of Interests & Access Points", "action", CH_1sub.id, function()
        menu.notify("Unlocked Successfully", "Diamond Casino", 5, 257818)
    ...
    menu.add_feature("» Silent and Sneaky Approach (Hard)", "action", CH_diff.id, function()
        menu.notify("Approach changed to Silent and Sneaky (Hard)", "Approach Editor", 5, 257818)
    ...
    menu.add_feature("» Silent and Sneaky Approach (Normal)", "action", CH_diff.id, function()
        menu.notify("Approach changed to Silent and Sneaky (Normal)", "Approach Editor", 5, 257818)
        for i = 1, #CH_Diff_Normal1 do
            stat_set_int(CH_Diff_Normal1[i][1], true, CH_Diff_Normal1[i][2])
        end
    end)
    
    menu.add_feature("» BigCon Approach (Hard)", "action", CH_diff.id, function()
        menu.notify("Approach changed to BigCon (Hard)", "Approach Editor", 5, 257818)
        for i = 1, #CH_Diff_Hard2 do
            stat_set_int(CH_Diff_Hard2[i][1], true, CH_Diff_Hard2[i][2])
        end
    end)
    
    menu.add_feature("» BigCon Approach (Normal)", "action", CH_diff.id, function()
        menu.notify("Approach changed to BigCon (Normal)", "Approach Editor", 5, 257818)
        for i = 1, #CH_Diff_Normal2 do
            stat_set_int(CH_Diff_Normal2[i][1], true, CH_Diff_Normal2[i][2])
        end
    end)
    
    menu.add_feature("» Aggressive Approach (Hard)", "action", CH_diff.id, function()
        menu.notify("Approach changed to Aggressive (Hard)", "Approach Editor", 5, 257818)
        for i = 1, #CH_Diff_Hard3 do
            stat_set_int(CH_Diff_Hard3[i][1], true, CH_Diff_Hard3[i][2])
        end
    end)
    
    menu.add_feature("» Aggressive Approach (Normal)", "action", CH_diff.id, function()
        menu.notify("Approach changed to Aggressive (Normal)", "Approach Editor", 5, 257818)
...
menu.add_feature("» Diamond", "action", CH_Target.id, function()
    menu.notify("Target changed to Diamond", "Target Editor", 5, 257818)
...
menu.add_feature("» Gold", "action", CH_Target.id, function()
    menu.notify("Target changed to Gold", "Target Editor", 5, 257818)
...
menu.add_feature("» Artwork", "action", CH_Target.id, function()
    menu.notify("Target changed to Artwork", "Target Editor", 5, 257818)
...
menu.add_feature("» Cash", "action", CH_Target.id, function()
    menu.notify("Target changed to Cash", "Target Editor", 5, 257818)
...
----
    ...
    local CH_2sub = menu.add_feature("» Heist Planning (Board 2)", "parent", CHMN.id)
    ...
    menu.add_feature("» Remove Gunman Member (0% Payout)", "action", CH_Gunman.id, function()
        menu.notify("Gunman Member Removed", "Gunman Member", 5, 75000)
    ...
    menu.add_feature("» Best Variation", "action", CH_Gunman_sub.id, function()
        menu.notify("Variation Changed to Best", "Weapon Loadout", 5, 257818)
    ...
    menu.add_feature("» Worst Variation", "action", CH_Gunman_sub.id, function()
        menu.notify("Variation Changed to Worst", "Weapon Loadout", 5, 257818)
    ...
    local CH_DRV_CREW = menu.add_feature("» Change Driver", "parent", CH_2sub.id)
    
    menu.add_feature("» Chester McCoy (10%)", "action", CH_DRV_CREW.id, function()
        menu.notify("Vehicle Variation: Fine\nVehicle: Vagrant (2 Players)\n\nVehicle Variation: Worst\nVehicle: Zhaba (4 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Vehicle Variation: Best\nVehicle: Everon (4Players)\n\nVehicle Variation: Good\nVehicle: Outlaw (2 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Chester McCoy\nCut: 10%", "Heist Control", 7, 98798561564)
    ...
    menu.add_feature("» Eddie Toh (9%)", "action", CH_DRV_CREW.id, function()
        menu.notify("Vehicle Variation: Fine\nVehicle: Gauntlet Classic (2 Players)\n\nVehicle Variation: Worst\nVehicle: Sultan Classic (4 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Vehicle Variation: Best\nVehicle: Komoda (4 Players)\n\nVehicle Variation: Good\nVehicle: Ellie (2 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Eddie Toh\nCut: 9%", "Heist Control", 7, 98798561564)
    ...
    menu.add_feature("» Taliana Martinez (7%)", "action", CH_DRV_CREW.id, function()
        menu.notify("Vehicle Variation: Fine\nVehicle: Drift Yosemite (2 Players)\n\nVehicle Variation: Worst\nVehicle: Retinue Mk II (2 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Vehicle Variation: Best\nVehicle: Jugular (4 Players)\n\nVehicle Variation: Good\nVehicle: Sugoi (4 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Taliana Martinez\nCut: 7%", "Heist Control", 7, 98798561564)
    ...
    menu.add_feature("» Zach Nelson (6%)", "action", CH_DRV_CREW.id, function()
        menu.notify("Vehicle Variation: Fine\nVehicle: Stryder (1 Player)\n\nVehicle Variation: Worst\nVehicle: Manchez (2 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Vehicle Variation: Best\nVehicle: Lectro (2 Players)\n\nVehicle Variation: Good\nVehicle: Defiler (1 Player)", "Heist Control", 9, 98798561564)
        menu.notify("Zach Nelson\nCut: 6%", "Heist Control", 7, 98798561564)
    ...
    menu.add_feature("» Karim Denz (5%)", "action", CH_DRV_CREW.id, function()
        menu.notify("Vehicle Variation: Fine\nVehicle: Asbo (2 Players)\n\nVehicle Variation: Worst\nVehicle: Issi Classic (2 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Vehicle Variation: Best\nVehicle: Sentinel Classic (2 Players)\n\nVehicle Variation: Good\nVehicle: Kanjo (2 Players)", "Heist Control", 9, 98798561564)
        menu.notify("Karim Denz\nCut: 5%", "Heist Control", 7, 98798561564)
    ...
    menu.add_feature("» Random Driver Member (??%)", "action", CH_DRV_CREW.id, function()
        menu.notify("Driver Randomized", "Heist Control - Driver Member", 4, 257818)
    ...
    menu.add_feature("» Remove Driver Member (0% Payout)", "action", CH_DRV_CREW.id, function()
        menu.notify("Driver Member Removed", "Heist Control - Driver Member", 4, 257818)
    ...
    menu.add_feature("» Best Variation", "action", CH_DRV_CREW_var.id, function()
        menu.notify("Best Variation Selected", "Heist Control - Vehicle Variation", 4, 257818)
    ...
    menu.add_feature("» Good Variation", "action", CH_DRV_CREW_var.id, function()
        menu.notify("Good Variation", "Heist Control - Vehicle Variation", 4, 257818)
    ...
    menu.add_feature("» Fine Variation", "action", CH_DRV_CREW_var.id, function()
        menu.notify("Fine Variation", "Heist Control - Vehicle Variation", 4, 257818)
    ...
    menu.add_feature("» Worst Variation", "action", CH_DRV_CREW_var.id, function()
        menu.notify("Worst Variation", "Heist Control - Vehicle Variation", 4, 257818)
    ...
    menu.add_feature("» Random Car Variation", "action", CH_DRV_CREW_var.id, function()
        menu.notify("Car Randomized", "Heist Control - Vehicle Variation", 4, 257818)
        for i = 1, #CH_DRV_MAN_var_RND do
    ...
    menu.add_feature("» Random Hacker Member (??%)", "action", CH_HCK_CREW.id, function()
        menu.notify("Hacker Member Randomized", "Heist Control - Hacker Member", 4, 257818)
    ...
    menu.add_feature("» Remove Hacker Member (0% Payout)", "action", CH_HCK_CREW.id, function()
        menu.notify("Hacker Member Removed", "Heist Control - Hacker Member", 4, 780000)
    ...
    menu.add_feature("» Remove Mask", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Removed", "Heist Control", 4, 75000)
    ...
    menu.add_feature("» Geometric Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Geometric Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Hunter Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Hunter Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Oni Half Mask Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Oni Half Mask Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Emoji Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Emoji Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Ornate Skull Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Ornate Skull Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Lucky Fruit Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Lucky Fruit Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Guerilla Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Guerilla Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Clown Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Clown Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Animal Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Animal Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Riot Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Riot Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Oni Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Oni Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Hocket Set", "action", CH_MASK_VAR.id, function()
        menu.notify("Mask: Hockey Set", "Heist Control", 4, 257818)
    ...
    menu.add_feature("» Unlock Scan Card (LVL 2)", "action", CH_2sub.id, function()
        menu.notify("Scan Card LVL 2 Unlocked", "Heist Control", 4, 257818)
        for i = 1, #CH_SCANC_LVL do
            stat_set_int(CH_SCANC_LVL[i][1], true, CH_SCANC_LVL[i][2])
        end
    end)
    
    menu.add_feature("» Weaken Duggan Heavy Guards", "action", CH_2sub.id, function()
        menu.notify("Duggan Heavy Guards Weakened", "Heist Control", 4, 257818)
...
do
    ...
    local CH_HEIST_BOARD3 = menu.add_feature("» Heist Planning (Board 3)", "parent", CHMN.id)

menu.add_feature("» Remove Drill for Silent and Aggressive Approach", "action", CH_HEIST_BOARD3.id, function()
    menu.notify("Drill removed for Silent and Aggressive Approach", "Heist Control", 4, 150000)
    for i = 1, #CH_UNLCK_3stboard_var1 do
        stat_set_int(CH_UNLCK_3stboard_var1[i][1], true, CH_UNLCK_3stboard_var1[i][2])
    end
end)

menu.add_feature("» Remove Drill for BigCon Approach only", "action", CH_HEIST_BOARD3.id, function()
    menu.notify("Drill removed for BigCon", "Heist Control", 4, 150000)
    ...
    local CH_PLAN_BOARD = menu.add_feature("» Board Loader/Unloader", "parent", CHMN.id)
    
    menu.add_feature("» Load all Boards", "action", CH_PLAN_BOARD.id, function()
        menu.notify("All Planning Board Loaded", "Heist Control", 4, 257818)
        for i = 1, #CH_LOAD_BOARD_var0 do
            stat_set_int(CH_LOAD_BOARD_var0[i][1], true, CH_LOAD_BOARD_var0[i][2])
        end
    end)
    
    menu.add_feature("» Unload all Boards", "action", CH_PLAN_BOARD.id, function()
        menu.notify("All Planning Board Unloaded", "Heist Control", 4, 75000)
...
do
    ...
    local MR_CAH_OPT = menu.add_feature("» More Options", "parent", CHMN.id)

menu.add_feature("» Unlock Casino Awards", "action", MR_CAH_OPT.id, function()
    menu.notify("Casino Heist Awards Unlocked", "Heist Control", 4, 257818)
...
------
    ...
    menu.add_feature("» Unlock Packie McReary", "action", MR_CAH_OPT.id, function()
        menu.notify("NOTE: it is experimental, it may not work correctly.", "Heist Control", 4, 780000)
    ...
    menu.add_feature("» Remove Repeat Cooldown", "action", MR_CAH_OPT.id, function()
        menu.notify("This is not a bypass for the server-side cooldown (payment)", "Heist Control", 5, 780000)
...
menu.add_feature("» Set Heist to Default (Reset)", "action", MR_CAH_OPT.id, function()
        menu.notify("Now call Lester and try to cancel the Casino Heist", "Heist Control", 4, 780000)
    for i = 1, #CH_RST do
...
menu.add_feature("» Remove all crew members (Crew Payout 0%)", "action", MR_CAH_OPT.id, function()
        menu.notify("Use after stealing the target, before leaving the tunnel\n\nAll Members Removed!", "Heist Control", 6, 780000)
...
menu.add_feature("» The Data Breaches: ACT1 (Final Heist)", "action", DDPR.id, function()
    menu.notify("The Data Breaches (ACT1)\nReady to play", "Heist Control", 4, 257818)
    for i = 1, #DD_H_ACT1 do
        stat_set_int(DD_H_ACT1[i][1], true, DD_H_ACT1[i][2])
    end
end)

menu.add_feature("» The Bogdan Problem: ACT2 (Final Heist)", "action", DDPR.id, function()
    menu.notify("The Bogdan Problem (ACT2)\nReady to play", "Heist Control", 4, 257818)
    for i = 1, #DD_H_ACT2 do
        stat_set_int(DD_H_ACT2[i][1], true, DD_H_ACT2[i][2])
    end
end)

menu.add_feature("» The Doomsday Scenario: ACT3 (Final Heist)", "action", DDPR.id, function()
    menu.notify("The Doomsday Scenario (ACT3)\nReady to play", "Heist Control", 4, 257818)
...
menu.add_feature("» Unlock all Doomsday Heist", "action", DHMN.id, function()
    menu.notify("Call the Lester and ask to cancel the Doomsday Heist (Three Times)\nDo this only once", "Heist Control", 6, 780000)
    for i = 1, #DD_H_ULCK do
        stat_set_int(DD_H_ULCK[i][1], true, DD_H_ULCK[i][2])
    end
end)

menu.add_feature("» Complete all preparations (Not setups)", "action", DHMN.id, function()
    menu.notify("All Preps are completed", "Heist Control", 4, 257818)
...
menu.add_feature("» Set Heist to Default (Reset)", "action", DHMN.id, function()
    menu.notify("Doomsday restored\nGo to a new session!!!", "Heist Control", 4, 8590000)
    for i = 1, #DD_H_RST do
...
do
    ...
    menu.add_feature("» Unlock Doomsday Heist Awards", "action", DHMN.id, function()
        menu.notify("Doomsday Awards Unlocked", "Heist Control", 4, 257818)
...
do
    ...
    menu.add_feature("» Complete all Setups", "action", HHMN.id, function()
    for i = 1, #Apartment_SetDone do
    menu.notify("Remember: First pay and watch/skip the first cutscene", "Heist Control", 5, 780000)
    stat_set_int(Apartment_SetDone[i][1], true, Apartment_SetDone[i][2])
end
end)


menu.add_feature("» Unlock High-End Apartments Heist Awards", "action", HHMN.id, function()
    menu.notify("Apartment Awards Unlocked", "Heist Control", 4, 257818)
...
menu.add_feature("» The Superdollar Deal", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_1_SD do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe Superdollar Deal Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» The Bank Contract", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_2_BC do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe Bank Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» The ECU Job", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_3_ECU do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe ECU Job Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» The Prison Contract", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_4_PRSN do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe Prison Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» The Agency Deal", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_5_AGC do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe Agency Deal Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» The Lost Contract", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_6_LOST do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe Lost Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» The Data Contract", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_7_DATA do
    menu.notify("Changes will only happen if you are outside your Auto-Shop!\n\n\nThe Data Contract ready to play", "Heist Control", 5, 257818)
...
menu.add_feature("» Complete missions only", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_MSS_ONLY do
    menu.notify("Changes will only happen if you are outside your Auto-Shop\n\nMissions completed!","Heist Control", 5, 257818)
...
menu.add_feature("» Reset Contracts", "action", CONTRACT_LS.id, function()
    for i = 1, #LS_CONTRACT_RST do
    menu.notify("Changes will only happen if you are outside your Auto-Shop\n\nContract reseted","Heist Control", 5, 257818)
...
local LS_TUNERS_PRICE_IT = {
    ...
    local TUNERS_DLC = menu.add_feature("» LS Tuners DLC", "parent", MUMN.id)
    ...
    menu.add_feature("» Unlock Vehicle Challenge & Trade Price", "action", TUNERS_DLC.id, function()
    ...
    menu.add_feature("» Unlock XMAS Liveries", "action", MUMN.id, function()
        menu.notify("All Liveries Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Refill Inventory", "action", MUMN.id, function()
        ...
        menu.notify("Inventory Restored", "Master Unlocker", 4, 257818)
        end
    end)
    
    menu.add_feature("» Unlock all Arena Wars Trophy and Toys", "action", MUMN.id, function()
        menu.notify("Arena Wars Unlocked", "Master Unlocker", 4, 257818)
    ...
    local NIGHT_C_UNLCKS = menu.add_feature("» NightClub", "parent", MUMN.id)
    
    menu.add_feature("» NighClub MAX Popularity", "action", NIGHT_C_UNLCKS.id, function()
        menu.notify("Nightclub Popularity Increased", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock NightClub Awards", "action", NIGHT_C_UNLCKS.id, function()
        menu.notify("NightClub Awards Unlocked", "Master Unlocker", 4, 257818)
    ...
    local PLAYER_MENTAL_CHECK = menu.add_feature("» Mental State", "parent", MUMN.id)

    menu.add_feature("» Maximum", "action", PLAYER_MENTAL_CHECK.id, function()
        menu.notify("Mental State modified to the maximum", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Half", "action", PLAYER_MENTAL_CHECK.id, function()
        menu.notify("Mental State modified to the Half", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Remove", "action", PLAYER_MENTAL_CHECK.id, function()
        menu.notify("Mental State has been removed", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock Arcade Trophys and Toys", "action", MUMN.id, function()
        menu.notify("Arcade Trophys Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Add cosmetics items in the Office/MC", "action", MUMN.id, function()
        menu.notify("Please, now sell anything and switch session.", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Remove Vehicle Sell Daily Limit", "action", MUMN.id, function()
        menu.notify("Cooldown Removed", "Master Unlocker", 4, 780000)
    ...
    menu.add_feature("» Unlock Don't Cross the Line Tee", "action", MUMN.id, function()
        menu.notify("Unlocked...You can buy it from any Clothing Store", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock Shotaro", "action", MUMN.id, function()
        menu.notify("Shotaro is now avaliable to purchase at Legendary Motorsport", "Master Unlocker", 4, 257818)
        for i = 1, #SHT_UNLK do
    ...
    menu.add_feature("» Summer 2020 Awards", "action", MUMN.id, function()
        menu.notify("Summer 2020 Awards Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock Yacht Missions", "action", MUMN.id, function()
        menu.notify("Yacht Missions Unlocked~s~", "Master Unlocker", 4, 257818)
        for i = 1, #Yacht_MS do
            stat_set_int(Yacht_MS[i][1], true, Yacht_MS[i][2])
        end
    end)
    
    local ALN_TT_UNLCK = menu.add_feature("» Alien Tatto (Illuminati)", "parent", MUMN.id)
    
    menu.add_feature("» Apply the tatto for my MALE Character", "action", ALN_TT_UNLCK.id, function()
        menu.notify("Tatto applied, please switch session or suicide for it to show up", "Master Unlocker", 4, 780000)
    ...
    menu.add_feature("» Skip Lamar Missions to the last one", "action", MUMN.id, function()
        menu.notify("Done, please switch session for it to take effect.", "Master Unlocker", 5, 780000)
    ...
    menu.add_feature("» Unlock Flight School Awards", "action", MUMN.id, function()
        menu.notify("Flight School Awards Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Set fast run and reload - ON", "action", FAST_RUN_M.id, function()
        menu.notify("Fast Run and Reload: Enabled", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Set fast run and reload - OFF", "action", FAST_RUN_M.id, function()
        menu.notify("Fast Run and Reload: Disabled", "Master Unlocker", 4, 75000)
    ...
    menu.add_feature("» Unlock some vehicles trade price", "action", MUMN.id, function()
        menu.notify("Trade Price Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock all Contacts", "action", MUMN.id, function()
        menu.notify("Contacts Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock Vanilla Unicorn Award", "action", MUMN.id, function()
        menu.notify("Vanilla Unicorn Awards Unlocked", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Trigger Alien Egg Mission", "action", BNKR_AWARDS.id, function()
        menu.notify("It's necessary that you change the clock time to between 9pm and 11pm", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock Bunker Awards", "action", BNKR_AWARDS.id, function()
        menu.notify("Bunker Awards Unlocked!", "Master Unlocker", 4, 257818)
    ...
    menu.add_feature("» Unlock All Daily Objectives Awards", "action", MUMN.id, function()
        menu.notify("Daily Objectives Awards Unlocked!", "Master Unlocker", 4, 257818)
...
local ORBT_CLDWN_ = {
    ...
    menu.add_feature("[!] Remove Orbital Cannon Cooldown [risky]", "action", MUMN.id, function()
        menu.notify("Abusing this option can lead to a ban!!!", "Warning", 7, 75000)
        menu.notify("Cooldown Removed!", "Master Unlocker", 7, 780000)
...
menu.add_feature("» Leave Session (Freeze the game for a few secs)", "action", MSC.id, function()
    menu.notify("Task Completed", "Heist Control", 3, 257818)
...
do
menu.add_feature("» About Heist Control", "action", HC2_MAIN.id, function()
    menu.notify("Thanks to\nhaekkzer\n2TAKE1 Menu Devs", "Heist Control 2.7.0", 5, 3578712200220)
    menu.notify("For future updates, visit:\ngithub.com/jhowkNx/Heist-Control-v2", "Heist Control - Official Page", 5, 257818)
end)
end

menu.notify("» To repeat the same Heist, you must wait up to 15 minutes (cooldown), or you won't get paid\n\n» Disable certain protections to not cause bugs while doing the Heist\n\n» Keep in mind that Rockstar employers can join your heist, be aware.", "", 6, 719841)
menu.notify("Version 2.7.0\nMade by jhowkNx","Welcome to Heist Control" , 4, 3578712200220)